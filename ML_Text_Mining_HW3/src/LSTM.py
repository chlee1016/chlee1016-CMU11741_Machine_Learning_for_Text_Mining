...
def get_model_lstm_pre_trained(embedding_matrix):
    model = Sequential()
    model.add(Embedding(vocab_size, embedding_vector_length, input_length=max_review_length,
...
hist_lstm = model.fit(X_train, y_train, validation_data=(X_test, y_test), shuffle=True, epochs=5, batch_size=16)
print("training time for lstm:", time.time()-start_time)
...
model = get_model_lstm_pre_trained(embedding_matrix)
start_time = time.time()
hist_lstm_pre_trained = model.fit(X_train, y_train, validation_data=(X_test, y_test), shuffle=True, epochs=5, batch_size=16)
print("training time for lstm with pre-trained:", time.time()-start_time)
# Final evaluation of the model
scores = model.evaluate(X_test, y_test, verbose=0)
print("Accuracy: %.2f%%" % (scores[1]*100))
visualize(hist_lstm_pre_trained)
